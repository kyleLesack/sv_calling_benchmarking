...
# Extract SVsim variant calls
def parse_svsim(svsim_variants):
	...
	for line in variants:
		if line[0] != "#":
			...
			info_line = line_split[7]
			info_line_split = info_line.split(";")
			variant_type = info_line_split[2]
			variant_type = variant_type.split("=")[1]

			end_coord = info_line_split[5]
			end_coord = end_coord.split("=")[1]
			variant_size = int(end_coord) - int(start_coord)
			abs_variant_size= abs(int(variant_size))
			precision = line_split[6]

			if precision == "PASS":
				if variant_type in SVSIM_SV_TYPES:
					if abs_variant_size >= args.minsize and abs_variant_size <= args.maxsize:
						bed_line = chromosome + "\t" + start_coord + "\t" + end_coord + "\t" + variant_name
						callerDict["TRUTH"][variant_type].append(bed_line)

					elif abs_variant_size > args.maxsize:
						bed_line = chromosome + "\t" + start_coord + "\t" + end_coord + "\t" + variant_name
						callerDict_exceeds_maximum["TRUTH"][variant_type].append(bed_line)
	...
	print("SVSIM variant types excluded: " + str(excluded_variant_types))
	return (callerDict, callerDict_exceeds_maximum)
...
elif args.sv_caller.lower() == "svsim":
	with open(args.input_vcf) as f:
		variants = f.readlines()[1:] # read variants from file, skip header
	svsim_bedlines = parse_svsim(variants)
	write_bed(svsim_bedlines[0])
	write_bed_exceeds_max_size(svsim_bedlines[1])
