...
parser.add_argument("input_dir", help="SVE vcf directory.")
args = parser.parse_args()

# Names of the FusorSV size bins
DEL_BIN_LABELS = ["100-400bp","400-600bp","600-950bp","950-1.2Kbp","1.2K-1.5Kbp","1.5K-1.9Kbp","1.9K-2.2Kbp","2.2K-2.9Kbp","2.9K-3.6Kbp","3.6K-4.8Kbp","4.8K-6.1Kbp","6.1K-9Kbp","9K-18.5Kbp","18.5K-100Kbp","100K-1Mbp"]
DUP_BIN_LABELS = ["100-1Kbp","1K-10Kbp","10K-50Kbp","50K-100Kbp","100K-250Kbp","250K-500Kbp","500K-1Mbp"]
INV_BIN_LABELS = ["100-2500bp","2500-3500bp","3500-45Kbp","45K-80Kbp","80K-115Kbp","115K-180Kbp","180K-260Kbp","260K-300Kbp","300K-375Kbp","375K-500Kbp","500K-1Mbp"]

# FusorSV size bins
DEL_BINS = [100,400,600,950,1200, 1500, 1900, 2200, 2900, 3600, 4800, 6100, 9000, 18500, 100000,1000000]
DUP_BINS = [100,1000,10000,50000,100000,250000,500000,1000000]
INV_BINS = [100,2500,3500,45000,80000,115000,180000,260000,300000,375000,500000,1000000]
...
# Read vcf file and, for each sv call, extract the chromosome, coordinates, name, type, and size
def process_sve_calls(sve_file,caller):
	with open(sve_file) as f:
		read_data = f.readlines()
		end_coord_pos = 0
		vcf_lines = []
		if caller == "BreakDancer" or caller == "cnMOPS" or caller == "CNVnator" :
			end_coord_pos = 0
		elif caller == "Delly":
			end_coord_pos = 7
		elif caller == "Hydra":
		...
		for line in read_data:
			if line[0] != "#": # If not preceded by a #, then it should be a line describing a variant
				try:
					...
					if variant_type == "<DEL>" or variant_type == "<DUP>" or variant_type == "<INV>" :
						chromosome = line_split[0]
						start_coord = line_split[1]
						variant_name = line_split[2]
						variant_info = line_split[7]
						...
						if caller == "Delly":
							if variant_info_split[7] == "IMPRECISE":
								end_coord = variant_info_split[6].split("=")[1]
								variant_size = int(end_coord) - int(start_coord)
								vcf_line = [chromosome,start_coord,end_coord, variant_name,variant_type, variant_size]

							else:
								end_coord = variant_info_split[end_coord_pos].split("=")[1]
								variant_size = int(end_coord) - int(start_coord)
								vcf_line = [chromosome,start_coord,end_coord, variant_name,variant_type, variant_size]
					...
					print("Unexpected error:", sys.exc_info()[0])
					print("Error occurred for vcf file: " + str(sve_file))
					print(line)
...
def find_vcfs(input_dir):
	for file in os.listdir(input_dir):
		try:
			if file.endswith("S0.vcf"):
				truth_file=(os.path.join(input_dir, file))
				truth_data = process_truth(truth_file)
			elif file.endswith("S4.vcf"):
				caller = "BreakDancer"
				sve_file=(os.path.join(input_dir, file))
				sve_data = process_sve_calls(sve_file,caller)
			elif file.endswith("S9.vcf"):
				caller = "cnMOPS"
				sve_file=(os.path.join(input_dir, file))
				sve_data = process_sve_calls(sve_file,caller)
			elif file.endswith("S10.vcf"):
				caller = "CNVnator"
				sve_file=(os.path.join(input_dir, file))
				sve_data = process_sve_calls(sve_file,caller)
			elif file.endswith("S11.vcf"):
				caller = "Delly"
				sve_file=(os.path.join(input_dir, file))
				sve_data = process_sve_calls(sve_file,caller)
			elif file.endswith("S17.vcf"):
				caller = "Hydra"
				sve_file=(os.path.join(input_dir, file))
				sve_data = process_sve_calls(sve_file,caller)
			elif file.endswith("S18.vcf"):
				caller = "Lumpy"
				sve_file=(os.path.join(input_dir, file))
				sve_data = process_sve_calls(sve_file,caller)
			...
			print("Unexpected error:", sys.exc_info()[0])
			print("Error occurred for vcf file: " + str(file))

	return (sve_data,truth_data,caller)
...
sve_data_processed = make_variant_dict(sve_data)
sve_deletions = sve_data_processed[0]
sve_duplications = sve_data_processed[1]
sve_inversions = sve_data_processed[2]
...
del_file_short_prefix = caller + "_del_"
dup_file_short_prefix = caller + "_dup_"
inv_file_short_prefix = caller + "_inv_"

del_file_long_prefix = caller + "_deletions"
dup_file_long_prefix = caller + "_duplications"
inv_file_long_prefix = caller + "_inversions"

# Write bed files for caller vcf files
write_separate_beds(sve_deletions[0], deletion_output_dir, del_file_short_prefix)
write_bed(sve_deletions[0], bed_output_dir,del_file_long_prefix)
write_separate_beds(sve_duplications[0], duplication_output_dir,dup_file_short_prefix)
write_bed(sve_duplications[0], bed_output_dir,dup_file_long_prefix)
write_separate_beds(sve_inversions[0], inversion_output_dir,inv_file_short_prefix)
write_bed(sve_inversions[0], bed_output_dir,inv_file_long_prefix)

# Write bed files for truth vcf files
write_separate_beds(truth_deletions[0], deletion_output_dir,"truth_del_")
write_bed(truth_deletions[0], bed_output_dir,"truth_deletions")
write_separate_beds(truth_duplications[0], duplication_output_dir,"truth_dup_")
write_bed(truth_duplications[0], bed_output_dir,"truth_duplications")
write_separate_beds(truth_inversions[0], inversion_output_dir,"truth_inv_")
write_bed(truth_inversions[0], bed_output_dir,"truth_inversions")

# Summarize results and write to disk
summary_output_dir = os.path.abspath(args.input_dir) + "/summary/"
summary_prefix = caller + "_deletions"
write_summary(sve_deletions[1][0],sve_deletions[2][0], summary_output_dir,summary_prefix)
