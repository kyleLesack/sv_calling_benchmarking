...
parser.add_argument("sv_caller", help="variant caller used to create vcf file. Use assemblytics, mumandco, pbsv, sniffles, or svim")
...
sniffles_SV_TYPES = ["INV", "DEL", "DUP", "INVDUP", "INV/INVDUP", "DEL/INV"] # Note: DUP/INS causes problems, as some have END coords that start before the start. DUP/INS is rare, so they were ignored altogether.
			...
			if variant_type == "IMPRECISE": # pbsv uses different columns for imprecise variant calls
				...
				if variant_type in PBSV_SV_TYPES:
					if variant_type =="cnv": # pbsv classifies some calls as cnv, which provide different metadata in the info column
						...
						if variant_type != "cnv":
							if abs_variant_size >= args.minsize and abs_variant_size <= args.maxsize: # Check if variant size is in the desired range
								bed_line = chromosome + "\t" + start_coord + "\t" + end_coord + "\t" + variant_precision + "\t" + variant_name + "\tCN:" + copy_number
								callerDict[filter_line][variant_type].append(bed_line) # Keep track of SV calls and their coordinates, precision, name, and copy number
						...
						if abs_variant_size >= args.minsize and abs_variant_size <= args.maxsize:
							...
							callerDict[filter_line][variant_type].append(bed_line) # Keep track of SV calls and their coordinates, precision, name, and support for the call
